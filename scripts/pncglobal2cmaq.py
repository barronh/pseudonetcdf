...
_GCLIST = ["NO2", "NO", "O3", "NO3", "OH", "HO2", "N2O5", "HNO3", "HONO",
           "PNA", "H2O2", "NTR", "ROOH", "FORM", "ALD2", "PAR", "CO", "MEPX",
           "FACD", "C2O3", "PAN", "PACD", "AACD", "PANX", "OLE", "ETH",
           "IOLE", "TOL", "CRES", "OPEN", "MGLY", "XYL", "ISOP", "SO2",
           "SULF", "ETHA", "BENZENE", "NH3", "SV_ALK", "SV_XYL1", "SV_XYL2",
           "SV_TOL1", "SV_TOL2", "SV_BNZ1", "SV_BNZ2", "SV_TRP1", "SV_TRP2",
           "SV_ISO1", "SV_ISO2", "SV_SQT", "HG", "HGIIGAS", "MACR", "MVK"]
_NRLIST = []
_AELIST = ["ASO4J", "ASO4I", "AALKJ", "AXYL1J", "AXYL2J", "AXYL3J", "ATOL1J",
           "ATOL2J", "ATOL3J", "ABNZ1J", "ABNZ2J", "ABNZ3J", "ATRP1J",
           "ATRP2J", "AISO1J", "AISO2J", "ASQTJ", "ACORS", "ASOIL", "AISO3J",
           "AOLGAJ", "AOLGBJ", "ANIJ", "ACR_IIIJ", "ACR_VIJ", "APBJ", "APBK",
           "ACDJ", "AMN_HAPSJ", "AMN_HAPSK", "APHGJ", "AORGPAJ", "AORGPAI"]
_NMLIST = ["NUMATKN", "NUMACC", "NUMCOR"]
_SFLIST = ["SRFATKN", "SRFACC", "SRFCOR"]
...
_CB05SPCS = r"""
        "PAR": {
            ...
            "comment6": "Half the carbon is assumed propene (1 PAR + 1 OLE)"
        },
        "ROOH": {
            "expression": "RIP",
            "outunit": "ppmV"
...
def makedefaulticon(metcroprops, args):
    ...
    if profilepath is not None:
        ...
        if not args.keepall:
            iconfile = getvarpnc(iconfile, GCLIST + AELIST + SFLIST + NMLIST)
        varlist = [varkey for varkey in iconfile.variables.keys(
        ) if varkey in (GCLIST + AELIST + SFLIST + NMLIST)]
        missingunits = [varkey for varkey, var in iconfile.variables.items(
        ) if var[:].ndim == 2 and varkey not in GCLIST + AELIST + NMLIST + SFLIST]
        missing = ', '.join(missingunits)
        GCLIST = missingunits + GCLIST
        if missing != '':
            warn(profilepath + ' contains ' + missing +
                 ' whose units are assumed ppmV')

        def getvals(varkey):
            if varkey not in iconfile.variables:
                warn(varkey + ' not found in icon profile path')
                return 1e-32
            invglvls = np.convolve(
                [.5, .5], metcroprops['VGLVLS'], mode='valid')
            outvglvls = np.convolve([.5, .5], iconfile.VGLVLS, mode='valid')
            invar = iconfile.variables[varkey]
            allvals = interpbound(invglvls, outvglvls, invar[:])[:, None, None].repeat(
    ...
    setattr(DI, 'VAR-LIST', ''.join([i.ljust(16)
                                     for i in varlist + ["AIRDEN"]]))
...
def makedefaultbcon(metbdyprops, args):
    ...
    if profilepath is not None:
        ...
        if not args.keepall:
            bconfile = getvarpnc(bconfile, GCLIST + AELIST + SFLIST + NMLIST)
        varlist = [varkey for varkey in bconfile.variables.keys(
        ) if varkey in (GCLIST + AELIST + SFLIST + NMLIST)]
        missingunits = [varkey for varkey, var in bconfile.variables.items(
        ) if var[:].ndim == 2 and varkey not in GCLIST + AELIST + NMLIST + SFLIST]
        missing = ', '.join(missingunits)
        if missing != '':
            warn(profilepath + ' contains ' + missing +
        ...
        def getvals(varkey):
            if varkey not in bconfile.variables:
                ...
                return 1e-32
            invglvls = np.convolve(
                [.5, .5], metbdyprops['VGLVLS'], mode='valid')
            outvglvls = np.convolve([.5, .5], bconfile.VGLVLS, mode='valid')
            invar = bconfile.variables[varkey]
            south = interpbound(invglvls, outvglvls, invar[:, 0])[
                :, None].repeat(metbdyprops['NCOLS'] + 1, 1)
            north = interpbound(invglvls, outvglvls, invar[:, 2])[
                :, None].repeat(metbdyprops['NCOLS'] + 1, 1)
            east = interpbound(invglvls, outvglvls, invar[:, 1])[
                :, None].repeat(metbdyprops['NROWS'] + 1, 1)
            west = interpbound(invglvls, outvglvls, invar[:, 3])[
                :, None].repeat(metbdyprops['NROWS'] + 1, 1)
    ...
    def getunit(varkey):
        if varkey in GCLIST:
            ...
            return 'm**2/m**3'
    # varkey_unit = [(i, 'ppmV') for i in GCLIST] +
    #               [(i, 'micrograms/m**3') for i in AELIST] +
    ...
    setattr(DB, 'VAR-LIST', ''.join([i.ljust(16)
                                     for i in varlist + ["AIRDEN"]]))
...
def repair_ae(f, myioo):
    ...
    NUMFAC = {}
    NUMFAC['ATKN'] = 1.0 / ((DGINIAT ** 3.0) *
                            exp((9.0 / 2.0) * ((log(SGINIAT)) ** 2.0)))
    NUMFAC['ACC'] = 1.0 / ((DGINIAC ** 3.0) *
                           exp((9.0 / 2.0) * ((log(SGINIAC)) ** 2.0)))
    NUMFAC['COR'] = 1.0 / ((DGINICO ** 3.0) *
                           exp((9.0 / 2.0) * ((log(SGINICO)) ** 2.0)))
    SRFFAC = {}
    SRFFAC['ATKN'] = pi / \
        (DGINIAT * exp((5.0 / 2.0) * ((log(SGINIAT)) ** 2.0)))
    ...
    if len(version_check) > 1:
        warn('Some variables from aerosol versions %s were not found' %
             ' and '.join([str(v) for v in version_check]))
    moment3 = dict([(k, zeros(data_shape, dtype='f'))
                    for k in 'ATKN ACC COR'.split()])
...
def makeregriddedbdy(metbdy, args, spcs):
    ...
    """
    lonlatcoords = '/'.join(['%f,%f' % (lon, lat) for lon, lat in zip(
        metbdy.variables['longitude'][:].ravel(), metbdy.variables['latitude'][:].ravel())])
    out = []
    for ND49, ND49_REGRID_BDY in zip(args.ND49, args.ND49_REGRID_BDY):
        outf = extract(getvarpnc(ND49, None), lonlatcoords,
                       method=args.extractmethod)
...
def makeregriddedcro(metcro, args, spcs):
    ...
    """
    lonlatcoords = '/'.join(['%f,%f' % (lon, lat) for lon, lat in zip(
        metcro.variables['longitude'][:].ravel(), metcro.variables['latitude'][:].ravel())])
    out = []

    for ND49, ND49_REGRID_CRO in zip(args.ND49, args.ND49_REGRID_CRO):
        outf = extract(slice_dim(getvarpnc(ND49, None), 'time,0'),
                       lonlatcoords, method=args.extractmethod)
        if args.persistintermediate:
...
def getdefault(oldcon, vark, noutstep):
    ...
    if defval.shape[0] == 1:
        ...
        warn('Default (I,B)CON has time dimension that is greater than output (%d>%d); only first %d are used.' % (
            defval.shape[0], noutstep, noutstep))
...
def makeibcon(args):
    ...
    if dobcon:
        ...
        metbdyfiles, metbdyargs = pncparse(
            has_ofile=False, plot_options=False, interactive=False, args=args.METBDY3D.split(' '), parser=None)
        try:
            metbdy = getvarpnc(metbdyfiles[0], ['PRES'])
        except Exception:
            metbdy = getvarpnc(metbdyfiles[0], None)
        add_cf_from_ioapi(metbdy)
        regridded_nd49_bdy = makeregriddedbdy(metbdy, args, spcs)

        metbdyprops = dict([(propk, getattr(metbdy, propk))
                            for propk in metbdy.ncattrs()])
        metbdyprops['VGLVLSCDL'] = 'f, '.join(
            ['%f' % vgl for vgl in metbdyprops['VGLVLS']]) + 'f'
        ...
        if args.BCON == 'dummybcon.nc':
            ...
            bconfiles, bconargs = pncparse(
                has_ofile=False, plot_options=False, interactive=False, args=args.BCON.split(' '), parser=None)
            oldbcon = bconfiles[0]
        ...
        for vark, oldv in oldbcon.variables.items():
            if vark == 'TFLAG':
                continue
            newv = newbcon.createVariable(
                vark, oldv.dtype.char, oldv.dimensions)
            for propk in oldv.ncattrs():
                propv = getattr(oldv, propk)
                setattr(newv, propk, propv)
        try:
            nbconoutsteps = sum([len(tmpf.dimensions['time'])
                                 for tmpf in regridded_nd49_bdy])
        except Exception:
            nbconoutsteps = sum([len(tmpf.dimensions['TSTEP'])
                                 for tmpf in regridded_nd49_bdy])
        if len(newbcon.dimensions['PERIM']) != metbdyprops['PERIM']:
            raise ValueError('Default (I,B)CON has different perimeter dimension (%d) than the output file (%d).' % (
                len(newbcon.dimensions['PERIM']), metbdyprops['PERIM']))
        obconsteps = len(oldbcon.dimensions['TSTEP'])
        if obconsteps != 1 and obconsteps < nbconoutsteps:
            raise ValueError(
                'Default BCON has time dimension that is less than output (%d<%d) and not 1. Use time-independent file or file with same times.' % (obconsteps, nbconoutsteps))

    if doicon:
        if args.verbose:
            print('Starting ICON Prep')
        metcrofiles, metcroargs = pncparse(
            has_ofile=False, plot_options=False, interactive=False, args=args.METCRO3D.split(' '), parser=None)
        try:
            metcro = slice_dim(getvarpnc(metcrofiles[0], ['PRES']), 'TSTEP,0')
        except Exception:
            metcro = slice_dim(getvarpnc(metcrofiles[0], None), 'TSTEP,0')
        add_cf_from_ioapi(metcro)
        regridded_nd49_cro = makeregriddedcro(metcro, args, spcs)
        metcroprops = dict([(propk, getattr(metcro, propk))
                            for propk in metcro.ncattrs()])
        metcroprops['VGLVLSCDL'] = 'f, '.join(
            ['%f' % vgl for vgl in metcroprops['VGLVLS']]) + 'f'
        metcroprops['GDTYP'] = metcro.GDTYP

        if args.ICON == 'dummyicon.nc':
            oldicon = makedefaulticon(metcroprops, args)
        else:
            iconfiles, iconargs = pncparse(
                has_ofile=False, plot_options=False, interactive=False, args=args.ICON.split(' '), parser=None)
            oldicon = iconfiles[0]
        ...
        for vark, oldv in oldicon.variables.items():
            if vark == 'TFLAG':
                continue
            newv = newicon.createVariable(
                vark, oldv.dtype.char, oldv.dimensions)
            for propk in oldv.ncattrs():
                propv = getattr(oldv, propk)
                setattr(newv, propk, propv)
        if len(newicon.dimensions['ROW']) != metcroprops['NROWS']:
            raise ValueError('Default (I,B)CON has different ROW dimension (%d) than the output file (%d).' % (
                len(newicon.dimensions['ROW']), metcroprops['NROWS']))
        if len(newicon.dimensions['COL']) != metcroprops['NCOLS']:
            raise ValueError('Default (I,B)CON has different COL dimension (%d) than the output file (%d).' % (
                len(newicon.dimensions['COL']), metcroprops['NCOLS']))
    ...
    infiles = []
    if dobcon:
        infiles += [(metbdy, regridded_nd49_bdy,
    ...
    for metfile, regridded_nd49, oldcon, newcon, noutstep in infiles:
        ...
        for vark, varo in newcon.variables.items():
            ...
            if vark in mappings:
                ...
                inunits = [nd49.variables[cn].units.strip()
                           for cn in coexpr.co_names if cn in nd49.variables]
                if len(inunits) == 0:
                    warn("Cannot evaluate any parts of '%s' = '%s'; using default file" % (
                        vark, expr))
                    oldkeys.append(vark)
                    minout = getdefault(oldcon, vark, noutstep)
                else:
                    inC = dict([(cn, getattr(nd49.variables[cn], 'carbon', 1.))
                                for cn in coexpr.co_names if cn in nd49.variables])
                    inkgpermole = dict([(cn, getattr(nd49.variables[cn], 'kgpermole', np.nan))
                                        for cn in coexpr.co_names if cn in nd49.variables])
                    ...
                    for cn in coexpr.co_names:
                        expr = re.compile(
                            r'(?P<spc>\b' + cn + r'\b)').sub(conv, expr)
                    ...
                    for ndi, nd49 in enumerate(regridded_nd49):
                        if not args.sigmaeta:
                            rpressv = eval(
                                mappings_file['PRESS']['expression'], None, nd49.variables)
                        ...
                        try:
                            ...
                            warn("Cannot evaluate part of '%s' = '%s'" %
                                 (vark, expr))
                            oldkeys.append(vark)
                        ...
                        for ti, temp_hour in enumerate(temp_val):
                            if args.verbose:
                                print('\nHour', ti, ':      ', end='')
                            if args.sigmaeta or args.sigma:
                                cpress = cpressv[:, None].repeat(
                                    out[0, 0].size, 1)
                            else:
                                cpress = cpressv[ti, :]
                                cpress = cpress.reshape(cpress.shape[0], -1)
                                assert(
                                    (np.diff(cpress, axis=0).mean(1) < 0).all())
                            if args.sigmaeta:
                                if hasattr(nd49, 'VGLVLS'):
                                    rpress = (
                                        (nd49.VGLVLS[:-1] + nd49.VGLVLS[1:]) * .5)[:, None].repeat(out[0, 0].size, 1)
                                else:
                                    rpress = ((nd49.variables['etam_pressure'] - newcon.VGTOP / 100) / (
                                        1013.25 - newcon.VGTOP / 100))[:, None].repeat(out[0, 0].size, 1)
                            else:
                                if mappings_file['PRESS']['outunit'] == 'Pa':
                                    ...
                                    warn("Assuming pressure is Pa, but got %s" %
                                         mappings_file['PSURF']['outunit'])
                                if args.sigma:
                                    rpress = (rpress - newcon.VGTOP) / (
                                        100 * nd49.variables['PSURF'][ti, 0][None].reshape(1, -1) - newcon.VGTOP)
                            rpress = rpress.reshape(
                                rpress.shape[0], cpress.shape[1])
                            assert((np.diff(rpress, axis=0).mean(1) < 0).all())

                            outvals = np.zeros(cpress.shape, dtype='f')
                            for pidx, (cp, rp, column) in enumerate(zip(cpress.T, rpress.T, temp_hour.T)):
                                outvals[:, pidx] = np.interp(
                                    cp[::-1], rp[::-1], column[::-1])[::-1]
                                if args.verbose:
                                    print(4 * '\b', end='')
                                    print('%3.0f%%' %
                                          (pidx / cpress.shape[1] * 100), end='')
                            out[toff + ti, :,
                                :] = outvals.reshape(*out.shape[1:])
                        toff = toff + ti + 1
                    minout = np.maximum(out, minval)
                    if args.verbose:
                        sys.stdout.write('\nSPC     0    25    50    75   100')
                        sys.stdout.write('\n%s %.5e %.5e %.5e %.5e %.5e' % tuple(
                            ['Raw'] + np.percentile(out, [0, 25, 50, 75, 100]).tolist()))
                        sys.stdout.flush()
                        sys.stdout.write('\n%s %.5e %.5e %.5e %.5e %.5e\n' % tuple(
                            ['Min'] + np.percentile(minout, [0, 25, 50, 75, 100]).tolist()))
                        sys.stdout.flush()
            else:
                oldkeys.append(vark)
                minout = getdefault(oldcon, vark, noutstep)
        ...
        if args.timeindependent:
            ...
            if args.tstep is None:
                try:
                    ...
                    raise ValueError(
                        'Unable to determine timestep from file; use --tstep option to assign manually')
            ...
            sdate = datetime.strptime(
                str(newcon.SDATE) + ' ' + '%06d' % newcon.STIME, '%Y%j %H%M%S')
            dt = timedelta(hours=newcon.TSTEP // 10000.) + (datetime.strptime('%06d' %
                                                                              (newcon.TSTEP % 10000.), '%H%M%S') - datetime(1900, 1, 1))
            tnow = sdate
            for ti in range(varo.shape[0]):
                jdate = int(tnow.strftime('%Y%j'))
                itime = int(tnow.strftime('%H%M%S'))
                tflag[ti, :, :] = np.array(
                    [[jdate, itime]]).repeat(newcon.NVARS, 0)
                tnow = tnow + dt
        newcon.sync()
...
class bpchwithstdatm(PseudoNetCDFFile):
    ...
    def __init__(self, bpath, vertgrid='GEOS-5-REDUCED', nogroup=True, verbose=False):
        ...
        pvar = infile.createVariable(
            'PSURF', 'f', ('time', 'layer1', 'latitude', 'longitude'))
        pvar.units = 'hPa'
        pvar[:] = 1013.25
        ...
        tvar = infile.createVariable(
            'TMPU', 'f', ('time', layer, 'latitude', 'longitude'))
        tvar.units = 'K'
        tvar[:] = np.array([287.7, 286.9, 286.0, 285.2, 284.3, 283.4,
                            282.6, 281.7, 280.7, 279.8, 278.9, 277.9,
                            276.8, 275.3, 273.6, 271.8, 270.0, 268.2,
                            265.8, 262.8, 259.7, 256.4, 252.9, 249.2,
                            245.2, 241.0, 236.4, 230.5, 223.6, 216.8,
                            216.6, 216.6, 216.6, 216.6, 216.6, 216.6,
                            216.6, 217.5, 219.8, 222.0, 225.3, 233.6,
                            250.5, 269.2, 260.0, 237.7,
                            214.3])[None, :NLAYS, None, None] * o
...
class benchmark(PseudoNetCDFFile):
    ...
    def __init__(self, bpath, vertgrid='GEOS-5-NATIVE'):
        ...
        group_varkeys = [('', 'time'), ('', 'latitude'), ('', 'longitude'), ('', 'hyam'), (
            '', 'hybm'), ('', 'etam_pressure'), ('PEDGE-$', 'PSURF'), ('DAO-3D-$', 'TMPU')]
        group_varkeys += [('IJ-AVG-$', k.replace('IJ-AVG-$_', ''))
                          for k in inf.variables.keys() if 'IJ-AVG-$' in k]
        getvarpnc_str = [((group + '_' + varkey) if group != '' else varkey)
        ...
        for group, varkey in group_varkeys:
            ...
            outvar = outf.createVariable(
                varkey, invar.dtype.char, invar.dimensions)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--withnco', action='store_true', dest='withnco',
                        default=False, help='Use NCO to make dummy BCON/ICON.')
    parser.add_argument('--BCON', default="dummybcon.nc", type=str,
                        help='path (or PseudoNetCDF commands) to an old BCON file (default dummybcon.nc - created on the fly)')
    parser.add_argument('--ICON', default="dummyicon.nc",
                        help='path (or PseudoNetCDF commands) to an old ICON file')
    parser.add_argument('--persist', dest='persistintermediate', action='store_true',
                        default=False, help='Save interpolated files (may have speed benefits)')
    parser.add_argument('--sigmaeta', action='store_true',
                        help='Use sigma from CMAQ and calculate sigma for GEOS-Chem from pure eta levels for interpolation')
    parser.add_argument('--sigma', action='store_true',
                        help='Use sigma from CMAQ and calculate sigma for GEOS-Chem from pressure levels for interpolation')
    parser.add_argument('--timeindependent', default=False,
                        action='store_true', help='Start date/time is any start date time')
    parser.add_argument('-d', '--sdate', default=None,
                        help='Start date in YYYY-MM-DD HH:MM:SS format')
    parser.add_argument('--METBDY3D', default=None, type=str,
                        help='path (or PseudoNetCDF commands) to a MCIP METBDY3D file')
    parser.add_argument('--METCRO3D', default=None,
                        help='path (or PseudoNetCDF commands) to a MCIP METCRO3D file')
    ...
    parser.add_argument('--GCNML', default=None,
                        help='Gas concentration namelist')
    parser.add_argument('--keepall', default=False, action='store_true',
                        help='Keep all species from profile data, even if not explicitly used')
    parser.add_argument('--BCONPROFILEPATH', default=None,
                        help='path to BCON profile.dat')
    parser.add_argument('--ICONPROFILEPATH', default=None,
                        help='path to ICON profile.dat')
    parser.add_argument('--tstep', default=None,
                        help='Time increment between ND49 files')
    parser.add_argument('--mapping', default='mappings.json',
                        help='Path to mappings file (i.e., json formatted dictionary); use --template to get a mapping')
    parser.add_argument('--outbfolder', default=".",
                        help='Path to output folder for BCON.')
    parser.add_argument('--outifolder', default=".",
                        help='Path to output folder for ICON.')
    parser.add_argument('--template', dest='template', default=None, choices=['cb6mp_ae6_aq', 'cb05tucl_ae6_aq', 'cb05tucl_ae5_aq',
                                                                              'saprc07tc_ae6_aq', 'saprc07tc_ae5_aq'], type=str, help='Print template mappings.json to stdout and exit (still requires ND49 argument)')
    parser.add_argument('--gc-version', dest='gcversion', default=9, choices=[
                        8, 9, 10, 11], type=int, help='GEOS-Chem version is used to determine SOA mapping. Above 8 (9-11) use the same SOA species.')
...
"""
    ...
    if not os.path.exists(args.mapping):
        parser.print_help()
        raise ValueError(
            "mappings.json must exist and can be created using the --template option")
    ...
    args.ND49_REGRID_BDY = [os.path.join(args.outbfolder, os.path.basename(
        p).replace('.nc', '') + '.BDY.nc') for p in args.ND49PATH]
    args.ND49_REGRID_CRO = [os.path.join(args.outifolder, os.path.basename(
        p).replace('.nc', '') + '.CRO.nc') for p in args.ND49PATH]
    args.NEWBCON = os.path.join(args.outbfolder, os.path.basename(
        args.ND49PATH[0].replace('.nc', '') + '.BCON.nc'))
    args.NEWICON = os.path.join(args.outifolder, os.path.basename(
        args.ND49PATH[0].replace('.nc', '') + '.ICON.nc'))
    makeibcon(args)
