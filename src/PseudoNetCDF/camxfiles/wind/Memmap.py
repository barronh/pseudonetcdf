...
class wind(PseudoNetCDFFile):
    ...
    def __decorator(self, k, pncfv):
        ...
        for k, v in decor.get(k, {'units': 'm/s', 'long_name': k, 'var_desc': k}).items():
            setattr(pncfv, k, v)
        return pncfv
    ...
    def __add_variables(self):
        ...
        self.variables['U'] = self.__decorator('U', PseudoNetCDFVariable(
            self, 'U', 'f', ('TSTEP', 'LAY', 'ROW', 'COL'), values=self.__memmap[out_idx == 1].reshape(tsteps, lays, 2, rows, cols)[:, :, 0, :, :]))
        self.variables['V'] = self.__decorator('V', PseudoNetCDFVariable(
            self, 'V', 'f', ('TSTEP', 'LAY', 'ROW', 'COL'), values=self.__memmap[out_idx == 1].reshape(tsteps, lays, 2, rows, cols)[:, :, 1, :, :]))
