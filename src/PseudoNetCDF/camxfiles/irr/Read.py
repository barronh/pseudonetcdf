...
class irr(PseudoNetCDFFile):
    ...
    def __readheader(self):
        ...
        """
        rffile = self._rffile
        self.runmessage = fromfile(rffile, dtype=dtype(
            [('SPAD', '>i'), ('NOTE', 'S80'), ('EPAD', '>i')]), count=1)[0]['NOTE']
        self.SDATE, self.STIME, self.EDATE, self.ETIME = fromfile(rffile, dtype=dtype(
            [('SPAD', '>i'), ('SDATE', '>i'), ('STIME', '>f'), ('EDATE', '>i'), ('ETIME', '>f'), ('EPAD', '>i')]), count=1)[0].tolist()[1:-1]
            ...
            self.grids.append(
                dict(
                    zip(
                        ...
                        fromfile(rffile, dtype=dtype([('SPAD', '>i'), ('orgx', '>f'), ('orgy', '>f'), ('ncol', '>i'), (
                            'nrow', '>i'), ('xsize', '>f'), ('ysize', '>f'), ('iutm', '>i'), ('EPAD', '>i')]), count=1)[0].tolist()[1:-1]
                    )
                )
            )
            ...
            self.padomains.append(
                dict(
                    zip(
                        ...
                        fromfile(rffile, dtype=dtype([('SPAD', '>i'), ('grid', '>i'), ('istart', '>i'), ('iend', '>i'), (
                            'jstart', '>i'), ('jend', '>i'), ('blay', '>i'), ('tlay', '>i'), ('EPAD', '>i')]), count=1)[0].tolist()[1:-1]
                    )
                )
            )
        self.NRXNS = fromfile(rffile, dtype=dtype(
            [('SPAD', '>i'), ('NRXNS', '>i'), ('EPAD', '>i')]), count=1)[0]['NRXNS']

        self._data_start_byte = rffile.tell()

        self._record_dtype = dtype([('SPAD', '>i'), ('DATE', '>i'), ('TIME', '>f'), ('PAGRID', '>i'), (
            'NEST', '>i'), ('I', '>i'), ('J', '>i'), ('K', '>i'), ('IRRS', '>f', self.NRXNS), ('EPAD', '>i')])
    ...
    def loadVars(self, start, n, pagrid=0):
        ...
        for rxn in range(start, end):
            key = 'RXN_%02d' % rxn
            variables[key] = PseudoNetCDFVariable(self, key, 'f', ('TSTEP', 'LAY', 'ROW', 'COL'), values=zeros(
                shape, 'f'), units='ppm/hr', var_desk=key.ljust(16), long_name=key.ljust(16))
        ...
        for ti, (d, t) in enumerate(self.timerange()):
            ...
            for rxn in range(start, end):
                variables['RXN_%02d' % rxn][ti, :, :, :] = temp[:, rxn -
                                                                1].reshape(nj, ni, nk).swapaxes(1, 2).swapaxes(0, 1)
