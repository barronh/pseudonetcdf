...
class PseudoNetCDFFileTest(unittest.TestCase):
    ...
    def testApplyAlongDimensionsConvolveWithMax(self):
        ...
        ancf = tncf.applyAlongDimensions(TSTEP=lambda x: np.convolve(x, np.ones(
            2, dtype='f') / 2., mode='valid')).applyAlongDimensions(TSTEP=np.max)
    ...
    def testInsertDimension(self):
        ...
        self.assertEqual(
            True, (nncf.variables['O3'][:, 0] == nncf.variables['O3'][:, 1]).all())
