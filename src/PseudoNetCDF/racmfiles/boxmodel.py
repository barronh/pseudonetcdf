...
def box_model_conc(conc_path, start_datetime):
    ...
    t_tflag = start_date + \
        vectorize(lambda h: timedelta(hours=float(h)))(
            conc[spc_names[0]].astype('d'))
...
def box_model_mrg(conc_path, irr_path, start_datetime):
    ...
    sorted_rxns = [k for k in irr.variables.keys()
                   if k not in ('TFLAG', 'TIME')]
    sorted_rxns.sort()
    sorted_spcs = [k for k in conc.variables.keys()
                   if k not in ('TFLAG', 'TIME')]
    sorted_spcs.sort()
    ...
    ipr_vals = retval.createVariable(
        'IPR', 'f', ('TSTEP', 'SPECIES', 'PROCESS'))
