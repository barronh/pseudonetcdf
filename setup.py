...
def find_data():
    ...
    for thisdir, itsdirs, itsfiles in walker:
        if thisdir != os.path.join('src',
            ...
            data.extend([os.path.join(thisdir[len(prefix) - 1:], f)
                         for f in itsfiles
...
long = """NetCDF, NCO, and CDO are fantastic softwares that I use, emulate,
 and admire. The primary drawbacks are restrictions on which scientific data
 sources they will and won't work with, and what types of operations they will
 and won't do. PseudoNetCDF was originally just a NetCDF-like interface for
 many data formats, but has grown to include amny functionalities from NCO
 and CDO. This is a platform independent, easy to install software to make
 scientific data manipulation easy."""
setup(name='PseudoNetCDF',
      ...
      scripts=['scripts/pncmadis2pnceval.py', 'scripts/pncaqsraw4pnceval.py',
               ...
               'scripts/pncdump', 'scripts/pncdump.py',
               'scripts/pnceval', 'scripts/pnceval.py', 'scripts/pncgen',
               'scripts/pncgen.py', 'scripts/pncglobal2cmaq.py',
               'scripts/pncload', 'scripts/pncmap.py', 'scripts/pncqq.py',
               'scripts/pncscatter.py', 'scripts/pncts.py',
               'scripts/pncvertprofile.py', 'scripts/pncview',
               'scripts/pncview.py', 'scripts/pncwindrose.py'],
      install_requires=['numpy>=1.2', 'netCDF4',
